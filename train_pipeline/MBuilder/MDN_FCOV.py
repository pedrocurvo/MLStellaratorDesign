...
class MDN_FCOV(nn.Module):
    def __init__(self, input_dim, output_dim, num_gaussians):
        super(MDN_FCOV, self).__init__()
        ...
        self.sigma_not_in_diagonal = nn.Linear(64, int(num_gaussians * (output_dim * (output_dim-1)) / 2))
        self.sigma_diag = nn.Linear(64, num_gaussians * output_dim)
        self.pi = nn.Linear(64, num_gaussians)
    
    def forward(self, x):
        x = self.shared_layers(x)
        # Mu
        mus = nn.Tanh()(self.mu(x))
        # Sigma
        ...
        sigmas_diag = nn.ELU()(self.sigma_diag(x)) + 1e-6 + 1
        # Pi
    ...
    def loss2(self, parameters, y_true):
        ...
        #log_gauss = torch.logsumexp(exponent, axis=1)
        # if (-torch.mean(log_gauss)).isnan():
        #     return -torch.mean(log_gauss) / -torch.mean(log_gauss) + 1e12
